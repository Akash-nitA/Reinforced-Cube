...
def get_angle(A, B, C):
    ...
    try:
        cos_angle = (math.pow(a, 2) + math.pow(b, 2) -
                     math.pow(c, 2)) / (2 * a * b)
    except ZeroDivisionError as e:
        log.warning(
            "get_angle: A %s, B %s, C %s, a %.3f, b %.3f, c %.3f" % (
                A, B, C, a, b, c)
        )
        raise e
...
class CustomContour(object):
    ...
    def is_square(self, target_area=None):
        ...
        if self.width is None:
            (self.width, self.height) = square_width_height(
                self.approx, self.debug)
...
class RubiksOpenCV(object):
    ...
    def remove_square_within_square_candidates(self):
        ...
        for con in self.candidates:
            ...
            if child:
                ...
                if self.debug:
                    log.info("con %s will be removed, has child %s" %
                             (con, child))
            else:
                if self.debug:
    ...
    def get_cube_size(self):
        ...
        for con in self.candidates:
            if con.is_square(self.median_square_area):
                ...
                log.debug("%s has %d row size, %d col size" %
                          (con, row_size, col_size))
        ...
        if self.debug:
            log.info(
                "cube size is %s, size_count %s" % (
                    self.size, pformat(size_count))
            )
    ...
    def set_contour_row_col_index(self, con):
        ...
        row_size = round(
            cube_height / float(self.median_square_width +
        ...
        col_size = round(
            cube_width / float(self.median_square_width +
                               self.black_border_width)
        )
    ...
    def analyze(self, webcam, cube_size=None):
        ...
        for component in zip(contours, hierarchy):
            con = CustomContour(
                self, index, component[0], component[1], self.debug)
        ...
        if self.remove_non_square_candidates():
            self.display_candidates(
                self.image, "50 post non-squares removal #1")

        """
        for con in self.candidates:
            child_index = con.heirarchy[2]
            parent_index = con.heirarchy[3]
            log.info("%s has child %s, parent %s" %
                     (con, child_index, parent_index))
        """
        ...
        if self.remove_contours_outside_cube(self.candidates):
            self.get_median_square_area()
            self.display_candidates(
                self.image, "110 post outside cube removal")
            ...
            if missing_count > 0:
                ...
                if missing:
                    ...
                    if webcam:
                        ...
                        raise Exception(
                            "Unable to extract image from %s" % self.name)
        ...
        for row in range(size):
            ...
            for col in range(size):
                square_indexes_for_row.append(
                    init_square_index + (row * size) + col)
...
class RubiksVideo(RubiksOpenCV):
    ...
    def analyze_webcam(self, width=352, height=240):
        ...
        while True:
            ...
            if self.size_static is not None:
                cv2.putText(
                    self.image,
                    "%dx%dx%d" % (self.size_static,
                                  self.size_static, self.size_static),
                    (10, 20),
